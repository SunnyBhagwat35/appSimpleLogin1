...
from app.config import ALIAS_LIMIT, PAGE_LIMIT, RZ_SECRET, RZ_YEARLY, RZ_MONTHLY, RZ_ID
...
@dashboard_bp.route("/fill", methods=["GET", "POST"])
def fill():
    if request.method == 'POST':
        ...
        up = request.form.get('updated_at')
        canurl = request.form.get('cancel_url')
        upurl = request.form.get('update_url')

        event = request.form.get('event_time')
        nextbilldate = request.form.get('next_bill_date')
        plan = request.form.get('plan')
        user_id = request.form.get('user_id')
        cre = request.form.get('created_at')
        # print(id, user_id)
        subscription = Subscription.create(cancel_url=canurl, update_url=upurl, subscription_id=subid,
                                           event_time=event, next_bill_date=nextbilldate, cancelled=False,
                                           plan=plan, user_id=user_id)
        Session.commit()
        return render_template("dashboard/filling.html", rz_id=RZ_ID, subid=subid)

    return render_template("dashboard/filling.html", rz_id=RZ_ID)
